...
class SwiftDataCacheCharactersDataSource: CacheCharacterDataSourceType {
    ...
    func saveCharactersList(_ charactersList: CharacterResult, currentPage: Int) async {
        ...
        for character in charactersList.result {
            ...
            let characterData = CharactersData(id: character.id, name: character.name, status: character.status,
                                               species: character.species, type: character.type,
                                               gender: character.gender, origin: nil,
                                               location: nil, image: character.image,
                                               episode: character.episode, url: character.url,
                                               created: character.created,
                                               charactersResultData: charactersResult)
            await container.insert(charactersDataList: characterData)
            let locationOrigin = LocationCharacterData(name: character.origin.name,
                                                       url: character.origin.url,
                                                       charactersData: characterData)
            let locationLocation = LocationCharacterData(name: character.location.name,
