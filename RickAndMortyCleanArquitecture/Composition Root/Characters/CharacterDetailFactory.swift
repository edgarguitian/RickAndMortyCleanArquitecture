...
class CharacterDetailFactory: CreateCharacterDetailView {
    func create(characterId: String) -> CharacterDetailView {
        return CharacterDetailView(viewModel: createViewModel(characterId: characterId), createEpisodeDetailView: EpisodeDetailFactory())
    }
    
    func createViewModel(characterId: String) -> CharacterDetailViewModel {
        return CharacterDetailViewModel(getSingleCharacter: createUseCase(), getSingleEpisode: createSingleEpisodeUseCase(), errorMapper: RickAndMortyPresentableErrorMapper(), characterId: characterId)
    }
    ...
    private func createEpisodeRepository() -> SingleEpisodeRepositoryType {
        return SingleEpisodeRepository(apiDataSource: createAPIEpisodeDataSource(), errorMapper: RickAndMortyDomainErrorMapper(), cacheDataSource: CharacterDetailFactory.createCacheEpisodeDataSource())
    }
